...
function construirHTML(C: any, row: any): string {
  ...
  const pasosBlock = (hip && !row.preliminar && hip.pasos && hip.pasos.length)
    ...
    : "";
  const otras = (row.hipotesis_otras || []).map((c: string) => (C.hipotesis || {})[c]).filter(Boolean);
  const otrosBlock = (!row.preliminar && otras.length)
    ? `<div style='margin:14px 0 4px'><div style='color:#1a1a2e;font-weight:700;font-size:14px;margin:10px 0 6px'>Además notamos que podría estar afectándote</div>${otras.map((h: any) => `<div style='border-left:3px solid #c9a94a;background:#fdf8ec;padding:11px 14px;border-radius:6px;margin:7px 0'><div style='color:#1a1a2e;font-weight:600;font-size:14px'>${h.nombre}</div><div style='color:#5a6472;font-size:13px;margin-top:3px'>${h.recomendacion}</div></div>`).join("")}</div>`
    : "";
